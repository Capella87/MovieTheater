...
public class ChooseAnotherMovie extends JDialog
{
     ...
     * Create the dialog.
     */
    public ChooseAnotherMovie(Database db, Object[] bookingInfo)
    {
        this.db = db;
        ...
        try
        {
            var getAltSchedule = new StringBuilder(
                    "SELECT m.name, s.theater_id, s.date, s.start_time, m.movie_id, s.schedule_id FROM schedules s, movies m " + "WHERE m.movie_id = s.movie_id ");
            
            getAltSchedule.append(" and m.movie_id != ");
            getAltSchedule.append(movieId);
            getAltSchedule.append(" ORDER BY s.date ASC, m.name ASC");
            ...
            } else
            {
                ...
                for (int i = 0; i < altScheduleCount; i++) // ?
                {
                    ...
                    this.altScheduleInfo.add(new Object[] { getSchedules.getInt(columnCount), getSchedules.getInt("s.theater_id"), getSchedules.getInt(columnCount - 1) }); // schedule_id, theater_id, movie_id 삽입
                    getSchedules.next();
                }
        ...
        {
            JLabel lblNewLabel = new JLabel("다른 영화를 선택하세요.");
        ...
        }
        {
            ...
            {
                JButton changeScheduleButton = new JButton("영화 변경");
                        ...
                        try
                        {
                            ...
                            } else
                            {
                                try
                                {
                                    ...
                                    getAltSchedule.append(", movie_id = ");
                                    getAltSchedule.append(altScheduleInfo.get(lastSelectedIdx)[2]);
